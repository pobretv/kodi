...
controlFile = os.path.join(xbmcvfs.translatePath('special://userdata/addon_data/plugin.video.pobretv/'), '1-1-0.pobretv')
...
def addDir(name,url,modo,iconimage,pagina=False,tipo=False,infoLabels=False,poster=False,visto=False, menuO=False,favorito=False, agendado=False, aseguir=False):
    ...
    if menuO:
        ...
        if visto == True:
            if tipo == 'filme' or tipo == 'episodio':
...
def log(msg, level=xbmc.LOGINFO):
    level = xbmc.LOGINFO
    #print('[POBRE.TV]: %s' % (msg))
    xbmc.log('[POBRE.TV]: %s' % (msg), level)
    try:
