...
def authTrakt():
    try:
        if getTraktCredentialsInfo():
            if controlo.yesnoDialog('Conta de Trakt já existe!', 'Deseja apagar?', '', 'Pobre.tv Trakt'):
...
def traktAuth():
    try:
        if not (__ADDON__.getSetting('utilizadorTrakt') == '' or __ADDON__.getSetting('tokenTrakt') == '' or __ADDON__.getSetting('refreshTrakt') == ''):
            dialog = xbmcgui.Dialog().yesno('Pobre.tv', u'Conta de Trakt já existe!', '', 'Deseja apagar?', u'Não', 'Sim')
            if dialog:
        ...
        if __ADDON__.getSetting('utilizadorTrakt') == '' or __ADDON__.getSetting('tokenTrakt') == '' or __ADDON__.getSetting('refreshTrakt') == '':
            dialog = xbmcgui.Dialog().yesno('Pobre.tv', u'1. Entrar: [COLOR blue]http://trakt.tv/pin/8928[/COLOR]', '2. Se pedido, autorizar o acesso da conta.', '3. Colocar o PIN.', 'Inserir PIN', 'Cancelar')
